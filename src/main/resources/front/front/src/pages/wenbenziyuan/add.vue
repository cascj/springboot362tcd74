...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="资源附件" prop="ziyuanfujian">
            <file-upload
            tip="点击上传资源附件"
            action="file/upload"
            :limit="1"
			:type="3"
            :multiple="true"
            :fileUrls="ruleForm.ziyuanfujian?ruleForm.ziyuanfujian:''"
            @change="ziyuanfujianUploadChange"
            ></file-upload>
          </el-form-item>  
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="文本内容" prop="wenbenneirong">
            <editor 
                ...
                v-model="ruleForm.wenbenneirong" 
                class="editor" 
                action="file/upload">
            </editor>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          ziyuanfujian: [
            { required: true, message: '资源附件不能为空', trigger: 'blur' },
          ],
          wenbenneirong: [
          ],
          faburiqi: [
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='ziyuanfujian'){
              this.ruleForm.ziyuanfujian = obj[o];
              this.ro.ziyuanfujian = true;
              continue;
            }
            if(o=='wenbenneirong'){
              this.ruleForm.wenbenneirong = obj[o];
              this.ro.wenbenneirong = true;
              continue;
            }
            if(o=='faburiqi'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`wenbenziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('wenbenziyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`wenbenziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      ziyuanfujianUploadChange(fileUrls) {
          this.ruleForm.ziyuanfujian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
